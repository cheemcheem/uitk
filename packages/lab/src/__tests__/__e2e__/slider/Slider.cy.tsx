...
describe("Given a pushable range slider", () => {
  it("WHEN moving a handle, it should push other handles", () => {
    ...
    cy.findAllByRole("slider").eq(0).realPress("ArrowRight");
...
describe("Given a non-pushable range slider", () => {
  it("WHEN moving a handle, it should be constrained by the handles next to it", () => {
    ...
    cy.findAllByRole("slider").eq(0).realPress("PageUp");
    cy.findAllByRole("slider").eq(0).realPress("ArrowUp");
    cy.findAllByRole("slider").eq(0).realPress("ArrowRight");
    cy.findAllByRole("slider").eq(0).realPress("End");
    cy.get("@changeSpy")
      .should("have.been.calledOnce")
      .and("been.calledWith", [3, 3, 7]);

    cy.findAllByRole("slider").eq(0).realPress("Home");
    cy.findAllByRole("slider").eq(0).realPress("PageDown");
    cy.findAllByRole("slider").eq(0).realPress("ArrowLeft");
    cy.findAllByRole("slider").eq(0).realPress("ArrowDown");
    cy.get("@changeSpy")
      .should("have.been.calledTwice")
